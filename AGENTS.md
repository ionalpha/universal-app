...
That runs typecheck + lint + arch + file-size + duplication + dead-code +
security + template identity. All must pass. Fix a boundary or size failure by
moving/splitting code, not by suppressing the rule.

The app's name is not a string you type. It lives in one table
(`scripts/identity.mjs`), is written by `pnpm rename` and read back by
`pnpm template`; hardcoding it anywhere else fails the gate.
